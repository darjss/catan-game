...
export default {
  server: {
    // Vercel AI Gateway key for the bot brains (typesafe-ai/jev). Optional:
    // without it bots fall back to simple heuristics. Get one at
    // https://vercel.com/ai-gateway — Jev is billed per input token only,
    // $0.042/1M.
    AI_GATEWAY_API_KEY: v.optional(v.string()),
  },
  client: {
    VITE_APP_NAME: v.optional(v.pipe(v.string(), v.minLength(1)), "Catan"),
  },
};

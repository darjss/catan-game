...
- Solid 2 components, reactivity, server functions: [.agents/skills/solidjs-v2/SKILL.md](.agents/skills/solidjs-v2/SKILL.md) (mirrored from tranzfer2). Its reference files describe rc.5 — verify version-sensitive claims against installed `solid-js` / `@solidjs/web` types and `node_modules/solid-js/CHEATSHEET.md`.
- Game rules come from the `catan-game-engine` npm package (`Game`, `canPerformAction`, `getAvailableActions`) — do not reimplement rules. Its `Board` uses `Map`s; convert them when crossing serialization boundaries.
- Bot decisions use TypeSafe AI's `typesafe-ai/jev` evaluation model via `experimental_evaluate` from the `ai` package, routed through Vercel AI Gateway (`AI_GATEWAY_API_KEY` server env). It answers typed choice/score/boolean questions about supplied state — it is not a chat model.
- External precedent checkouts (mixed prerelease versions — mine shape, not syntax): `~/dev/tranzfer2-references/README.md`, especially `solid2-apps/melty-karts` for bridging a non-reactive engine into Solid and `gitcafe/quoin/internal-docs/solid-2.0/` for Solid 2 migration notes.
